...
TYPE :: Allowed_peaks
   PRIVATE
   TYPE(Surface):: surf
   CLASS(Initatom),ALLOCATABLE:: inicond
   CLASS(CRP3D),ALLOCATABLE:: thispes
   REAL(KIND=8):: E
...
!=======================================================
CONTAINS
SUBROUTINE INITIALIZE_ALLOWEDPEAKS(this)
   ...
   ! Local variables
   ! Run section
   CALL this%surf%INITIALIZE("INsurface.inp")
   CALL this%thispes%INITIALIZE("INcrp3d.inp")
   CALL this%inicond%INITIALIZE("INinicond3d.inp")
   RETURN
END SUBROUTINE 
!######################################################
...
SUBROUTINE SETUP_ALLOWEDPEAKS(this)
   ...
   mass=this%thispes%atomdat(1)%getmass()
	...
	CALL VERBOSE_WRITE(routinename, "Conic A: ", this%conic%A)
	CALL VERBOSE_WRITE(routinename, "Conic B: ", this%conic%B)
	CALL VERBOSE_WRITE(routinename, "Conic C: ", this%conic%C)
	CALL VERBOSE_WRITE(routinename, "Conic D: ", this%conic%D)
	CALL VERBOSE_WRITE(routinename, "Conic E: ", this%conic%E)
	CALL VERBOSE_WRITE(routinename, "Conic F: ", this%conic%F)
#endif
	! operations
	OPEN(11,FILE="allowed_peaks.out", STATUS="replace")
...
SUBROUTINE ASSIGN_PEAKS_TO_TRAJS_ALLOWEDPEAKS(this)
   ...
   INTEGER(KIND=4) :: ioerr
	...
	!---------
   OPEN(12,FILE="mapping_trajs-peaks.out",STATUS="replace")
   ...
   END DO
	...
	CLOSE(12)
	CLOSE(11)
	OPEN(13,FILE="OUTseen_peaks.out",STATUS = "replace") ! re-write allowed peaks file with probabilities printed
