MODULE H2onLiF001PES
#include "preprocessoptions.inc"
      use constants    !!!
      use PES_H2LiF001_MOD
      !implicit real(8) (a-h,o-z)
      Implicit none
      ! Logical to indicate whether this module has already been setup or not.
      logical, private :: PesModule_Setup = .FALSE.
      character(LEN=*), parameter, private :: moduleName = "H2onLiF001PES"
      type(Pes_H2LiF001):: thisPes


      !----------------------------------------------------
      ! The surface parameters for this system.
      !----------------------------------------------------
      real, parameter, private :: ZAsymptotic = 13.2280829302 ! 7 angst 
      real, parameter, private :: xLatticeConstant = 5.4433561 ! 2.8805 angst 
      real, parameter, private :: yLatticeConstant = 5.4433561 ! 2.8805 angst
      real, parameter, private :: skewingAngle = Constants_PI / 2.0
      character(LEN=*), parameter, private :: pointGroup = 'C4v'
...
!=======================================================================
      ...
      !-------------------------------------------------------
      ! Call this initialisation routine before any use !!!!
      !-------------------------------------------------------
      SUBROUTINE initThePESForH2LiF001()

        implicit none

        ! If already setup, return.
        IF ( PesModule_Setup ) RETURN

        call thisPes%initialize()
        ! Indicate that this module has now been setup.
        PesModule_Setup = .TRUE.

      END SUBROUTINE initThePESForH2LiF001

      !-------------------------------------------------------------
      ! Returns the surface parameters particular for this system.
      !-------------------------------------------------------------
      SUBROUTINE getParamsForH2LiF001( xlat_const, ylat_const, Z_asymptotic, &
                                      skewing_angle, point_group )

        implicit none
        real, intent(out)             :: Z_asymptotic
        real, intent(out)             :: xlat_const, ylat_const
        real, intent(out)             :: skewing_angle
        character(LEN=*), intent(out) :: point_group

        ! Return system parameters.
        Z_asymptotic  = ZAsymptotic
        xlat_const    = xLatticeConstant
        ylat_const    = yLatticeConstant
        skewing_angle = skewingAngle
        point_group   = pointGroup

      END SUBROUTINE getParamsForH2LiF001

      !-----------------------------------------------------------------------------------
      ! Returns the uncut potential in eV for the coordinates passed. The (x,y)
      ! coordinates passed refer to the skewed coordinate system of the unit
      ! cell.
      ! They are transformed to the Cartesian coordinate system used by this PES
      ! module.
      !-----------------------------------------------------------------------------------
      FUNCTION getVH2LiF001( x, y, Z, r, theta, phi, ZIndex, RIndex ) RESULT(POTV )

        implicit none
        real, intent(in)    :: x, y, Z, r, theta, phi
        integer, intent(in) :: ZIndex, RIndex
        real                :: POTV

        ! Local variables.
        !real :: pos(6), v, derivs
        real  :: pos(6), v, derivs(6)

        ! If this module has not already been setup, do it now.
        IF ( .NOT. PesModule_Setup ) CALL initThePESForH2LiF001()

        ! Put the coordinates in vector pos.
        !RAB = (/ x, y, Z, r, theta, phi /)
        pos = (/ x, y, Z, r, theta, phi  /)

        ! Compute the potential (and its derivatives).

        call thisPes%get_v_and_derivs(pos,v,derivs)
        ! Return potential energy in eV
        POTV = v*27.211383

      END FUNCTION getVH2LiF001

END MODULE H2onLiF001PES
...
function termFoup4mm(this,k,parity,irrep,x) result(answer)
   ...
   ! Run section -------------------------------------------
   g=2.D0*PI/5.4433561257770959d0
   select case( irrep )
   case('A1')
      select case( parity )
      case('+')
...
subroutine initialize_PES_HLIF001_NS(this,filename,tablename)
   ...
   allocate( this%irrepList(6) )
   this%irrepList(:)='A1'
   allocate( this%parityList(6) )
   this%parityList(:)='+'
...
!> @brief
!! Provides generic routines to interpol a H2LiF001 PES
!##########################################################
...
SUBROUTINE PRINT_INPUT_CUT2D(this,filename)
   ...
   OPEN (10,FILE=filename,STATUS="replace",ACTION="write")
   WRITE(10,*) "# FILE generated by PRINT_CUT2D_INPUT H2LiF001"
   WRITE(10,*) this%alias
   WRITE(10,*) "au au rad"
...
!#######################################################
!> @brief
!! H2LiF001 PES explicit implementation
!#######################################################
module PES_H2LiF001_MOD
! Initial declarations
use UNITS_MOD, only: Length, pi
...
subroutine initialize_PES_H2LiF001(this,filename,tablename)
   ...
   this%totalTerms=10
   ! XY interpol
   allocate( this%irrepListXY(10) )
   this%irrepListXY(:)=['A1','A1','A1','A1','A1','B1','B1','A1','B2','B2']
   allocate( this%parityListXY(10) )
   this%parityListXY(:)=['+','+','+','+','+','+','+','+','+','+']
   allocate( this%kListXY(10,2) )
   this%kListXY(:,1)=[0,0,1,1,1,1,1,2,1,1]
