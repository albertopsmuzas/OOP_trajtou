program trajtouGetGraph_Phicut_crp6d
   ...
   select case(command_argument_count())
      case(7)
         ...
         call myPes%plot1d_phi(npoints=1000,X=initPoint,filename=trim(auxString))
         call etime(timeArr,timer)
      ...
      case(6)
         ...
         read(auxString,*) initPoint(5)
         call etime(timeArr,timer)
         call myPes%PLOT1D_PHI(npoints=1000,X=initPoint,filename='phiCut_crp6d.dat')
         call etime(timeArr,timer)

      case default
         write(0,*) 'ERR: bad number of arguments: ',command_argument_count()
         write(0,*) 'Expected number of arguments: 7 or 6'
         write(0,*) 'Case 7: Lua config file; initial X,Y,Z,R,THETA and graph name'
         write(0,*) 'Case 6: Lua config file; initial X,Y,Z,R and THETA'
         call exit(1)
   end select
...
end program trajtouGetGraph_Phicut_crp6d
